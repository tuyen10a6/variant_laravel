...
@section('content')
    <h3 class="ml-3 mt-5 mb-2">BẢNG GIÁ TRỊ THUỘC TÍNH</h3>
    <a class="btn btn-danger ml-3 mb-3" href="{{route('admin.attributive.create')}}">Thêm mới</a>
  ...
  @foreach($attribute_value as $item)
      <tr>
          ...
          <td  style=" display: flex; flex-wrap: wrap   ">
              <a href="{{route('admin.attributive.show', $item->id)}}"  style="margin-right:20px;text-decoration:none" >
              ...
              <a onclick="return confirm('Bạn có muốn xóa giá thị thuộc tính trên không?')" style="margin-right: 20px">
                  <form method="post" action="{{route('admin.attributive.remove', $item->id)}}">
                      @csrf
                      @method('DELETE')
