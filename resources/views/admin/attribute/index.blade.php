...
@section('content')
    <h3 class="ml-3 mt-5 mb-2">BẢNG THUỘC TÍNH</h3>
    <a class="btn btn-danger ml-3 mb-3" href="{{route('admin.attribute.create')}}">Thêm mới</a>
    <table class="table table-striped table-bordered">
        <thead>
        <tr>
            <th scope="col">STT</th>
            <th scope="col">attribute_code</th>
        ...
        <tr>
            ...
            <td>{{ $item->name }}</td>
            <td >
                <a href="{{route('admin.attribute.show', $item->id)}}"  style="margin-right:20px;text-decoration:none" >
                ...
                <a  onclick="return confirm('Bạn có muốn xóa thuộc tính trên không?')" style="margin-right: 20px; display: inline-flex">
                    <form method="post" action="{{route('admin.attribute.delete', $item->id)}}">
                        @csrf
                        @method('DELETE')
